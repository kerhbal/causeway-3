...
public class SimpleObjects {
    ...
    public void batchCreate() {
        List<SimpleObject> listToCreate = new ArrayList<>();
        for (int i = 0; i < 2000; i++) {
            String newName = "name" + i;
            listToCreate.add(SimpleObject.withName(newName));

        }
        simpleObjectRepository.saveAll(listToCreate);
    }
    ...
    @Action(semantics = SemanticsOf.SAFE)
    public SimpleObjectViewModel listAllViewModel(
    ) {
